...
| `admin_session_bootstrap` | `GET /admin/auth/session` | session cookieのみ。 | `200 {"authenticated":true,"csrf_token":"..."}`。 | sessionなし・無効は`200 {"authenticated":false}`（該当cookie削除）。保護記録不可は`503 authentication_unavailable`。 |
