...
| `repository` | PostgreSQL、Codex app-server JSON-RPC、時刻をusecase portとして実装する。 |
| `cmd` | Codex app-serverの実行可能ファイル・専用workdir設定を読取り検証し、外部adapterへ注入する。資格情報は読取らない。 |
| Frontend | 同一origin HTTP契約だけで生成状態・失敗要約・候補参照を表示する。 |

Codex app-serverはrepositoryだけが利用する。processの所有、ローカルのv2 schemaで確認した`thread/start`、`turn/start`、`item/completed`、`turn/completed`のwire、出力選択、終了手順は[Codex app-server adapter契約](design/codex-app-server-adapter.md)を正本とする。JSON-RPC message、thread/turn ID、モデル設定、接続資格情報、外部詳細エラーをusecase・domain・Browser・候補へ出さない。
...
外部I/Oを含むため、DB transactionをCodex呼出し中に保持しない。要求・1〜3回目の失敗試行は個別transactionで記録し、成功試行、候補、要求成功状態は同一transactionで確定する。成功候補がない最終失敗では、4回目の試行記録と要求失敗状態を同一transactionで確定する。

## Interfaces / Data

| port | 入力 | 出力・規則 |
| --- | --- | --- |
| CandidateGeneration | 完成済みの初回または修正プロンプト | HTML本文1件、または安全な`generation_unavailable`分類。app-serverの内部IDや詳細エラーは返さない。 |
| HTMLFormatValidator | HTML本文 | `valid`または`invalid_html`。 |
| GenerationStore | 要求、試行、候補 | [DB schema](design/database-schema.md)どおりにtransaction境界を守る。 |
...
- 取得接続不能、途中切断、app-serverの異常完了、HTML抽出不能は`generation_unavailable`として扱う。[adapter契約](design/codex-app-server-adapter.md)の一process/attempt、固定wire、通知不整合、HTTP切断、shutdown cleanupの規則に従う。
...
FEAT-001の管理read/mutation guardを全operationへ適用する。`cmd`はCodex app-serverの実行設定だけをServer側で扱い、資格情報は専用service accountのapp-server実行環境だけが扱う。app-serverは空の専用作業領域と最小権限で起動し、生成に不要なアプリのsource、DB、Secret、管理sessionへ到達させない。通常CIではCodex app-serverのtest doubleを使い、実接続は秘密を隔離した環境のsmoke testに限定する。
