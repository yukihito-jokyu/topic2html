...
- 成功出力: `authenticated: true`と、その画面表示中に限って使用するCSRF tokenを返す。許可メール、Google Token、session参照値、内部期限値を返さない。`Cache-Control: no-store`を必須とする。
...
- この操作は業務状態もsessionのアイドル期限も変更しない。
